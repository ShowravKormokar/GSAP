...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let ticking = false;
    let mouseTimeout = null; // Fixed variable name
    ...
    const animatePreview = () => {
        ...
        if (lastMousePosition.x < awardsListRect.left ||
    ...
    // Add hover events to each award
    awardsElements.forEach((award, index) => {
        const wrapper = award.querySelector(".award-wrapper");
        let currentPosition = POSITIONS.TOP;

        // Fixed event name from "mouseLeave" to "mouseleave"
        ...
        award.addEventListener("mouseleave", (e) => { // Fixed event name
            activeAward = null;
            const rect = award.getBoundingClientRect();
